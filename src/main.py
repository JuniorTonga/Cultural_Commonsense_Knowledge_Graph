...
def generate_cultural_commonsense(args,location, sub_topic, action=None,knowledge=None,if_then_event=None):
    if args.mode=='monolingual_setting':
        if args.action=='initial_generation':
            ...
            prompt=prepare_prompt(key_word,location=location,sub_topic=sub_topic,language='English')
    ...
    if len(structure_data)==2:
        merge=structure_data[0]+structure_data[1]
...
def extend_relation(model,initial_commonsense_data,args):
    ...
    for iter in range(steps):
         # First extension : we extend just relation xNext and oNext
        if iter==0:
            intermediate_all_new_knowledges, iter_all_new_knowledges=process_first_expand_iteration(data,args)
            ...
            if iter < steps-1:
                intermediate_all_new_knowledges,iter_all_new_knowledges=expand_knowledge(list_of_knowledge_to_expand,args)
                #update intermediate knowledge in the dataset
                new_rows=np.array(intermediate_all_new_knowledges)
                data=np.vstack((data,new_rows))
    return data
    ...
    if args.action=='initial_generation':
        for index in range(args.number_location):
            ...
            for subTopic in sub_topics:
                print(subTopic)
                for _ in range(0,1):
                    result=generate_cultural_commonsense(args,location,subTopic)
                    if result:
                        commonsense=result
        ...
        for index in range(args.number_location):
            ...
            if args.sub_sample:
                ...
                #initial_commonsense_data=x_next_df.head(1)
